...
def load_all_data_sameloc(path):
    """
    Note: reads dcm files and selects the ones that fall into specific location
    """
...
def load_all_data_samesubj(path):
    """
    """
    files = _input_files(path)
    max_z = [0, '']
    ...
    for f in files:
        ...
        for i in files[f]['files']:
            if i.endswith('00000.dcm'):
                ...
                identity = ds.get('PatientID')
                identity_nr = re.findall(r'\d+$', identity)[0]
                location = ds.get('id', "(missing)")
                if location < 0:
                    img_raw = ds.pixel_array
                    images.append([img_raw, identity_nr])
                    # print either id or location 
                    #images.append([img_raw, location])
                    print(ds[0x20,0x32].value)
    image_array = np.asarray(images)
    return image_array
...
def visualize(image_array):
    ...
    for i in range(1, columns*rows +1):
        ...
        a.set_title("location "+str(image_array[index][1]))  # set title
        plt.imshow(img)
        counter += 1
    plt.tight_layout()
    plt.show()
...
if __name__ == '__main__':
    ...
    array = load_all_data_sameloc(args.path)
    visualize(array)