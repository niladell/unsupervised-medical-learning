...
def input_fn(params):
    ...
    def gen():
        ...
        data_file = np.load('dsprites_ndarray.npz')
        data = data_file['imgs']
        for image in data:
            ...
            yield image, random_noise, 0

    dataset = tf.data.Dataset.from_generator(
                                gen,
                                output_types=(tf.float32, tf.float32, tf.float32),
                                output_shapes=(tf.TensorShape([HEIGHT, WIDTH, CHANNELS]), tf.TensorShape([noise_dim]), tf.TensorShape([]))
                            )
    ...
    image, random_noise, labels = dataset.make_one_shot_iterator().get_next()
    features = {
            'real_images' : image,
